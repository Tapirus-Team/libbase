...
namespace base
{
    ...
    public:
        ...
        RegKey(_In_ HKEY rootkey, _In_ const char* subkey, _In_ REGSAM access);
        ...
        bool HasValue(_In_ const char* value_name) const;
        ...
        // using it.
        HRESULT DeleteKey(_In_ const char* name);

        // Deletes an empty subkey. If the subkey has subkeys or values then this
        // will fail.
        HRESULT DeleteEmptyKey(_In_ const char* name);

        // Deletes a single value within the key.
        HRESULT DeleteValue(_In_ const char* value_name);

        // Getters:

        // Reads a REG_DWORD (uint32_t) into |out_value|. If |name| is null or empty,
        // reads the key's default value, if any.
        HRESULT ReadValue(_In_opt_ const char* name, _Out_ DWORD* out_value) const;

        // Reads a REG_QWORD (int64_t) into |out_value|. If |name| is null or empty,
        // reads the key's default value, if any.
        HRESULT ReadValue(_In_opt_ const char* name, _Out_ int64_t* out_value) const;
        ...
        HRESULT WriteValue(_In_opt_ const char* name, _In_ DWORD in_value);

        // Sets an int64_t value.
        HRESULT WriteValue(_In_opt_ const char* name, _In_ int64_t in_value);

        // Sets a string value.
        HRESULT WriteValue(_In_opt_ const char* name, _In_ const char* in_value);
